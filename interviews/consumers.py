...
class InterviewConsumer(AsyncWebsocketConsumer):
    ...
    async def handle_begin_interview(self):
        ...
        elif self.session.status != 'ongoing':
            await self.send(text_data=json.dumps({'type': 'error', 'message': 'Session must be started first'}))
            return

        session_config = await self.get_session_config()
        user_profile = await self.get_user_profile()

        await self.send(text_data=json.dumps({'type': 'interviewer_typing', 'message': 'Preparing first question...'}))

        opening_message = await database_sync_to_async(get_interviewer_response)(
    ...
    async def handle_answer(self, answer):
        from agents.interview_agent import get_interviewer_response, evaluate_answer
        self.session = await self.get_session()

        if self.session.questions_asked >= self.session.total_questions:
            await self.send(text_data=json.dumps({'type': 'interview_complete', 'message': 'Interview is complete!'}))
            return

        last_question = await self.get_last_question()
        await self.send(text_data=json.dumps({'type': 'evaluating', 'message': 'Evaluating your answer...'}))

        evaluation = None
        if last_question:
            evaluation = await database_sync_to_async(evaluate_answer)(
        ...
        await self.save_candidate_message(answer, evaluation['score'] if evaluation else None, evaluation['feedback'] if evaluation else None)
        ...
        if self.session.questions_asked >= self.session.total_questions:
            await self.send(text_data=json.dumps({'type': 'interview_complete', 'message': 'Interview complete!'}))
            return

        await self.send(text_data=json.dumps({'type': 'interviewer_typing', 'message': 'Preparing next question...'}))
        
        session_config = await self.get_session_config()
        user_profile = await self.get_user_profile()
        history = await self.get_conversation_history()

        next_question = await database_sync_to_async(get_interviewer_response)(
            session_config=session_config,
            user_profile=user_profile,
    ...
    async def handle_end_interview(self):
        ...
        await self.send(text_data=json.dumps({'type': 'session_ended', 'message': 'Interview session ended successfully'}))
    ...
    def get_user_from_token(self, token):
        ...
        except (TokenError, User.DoesNotExist): return None

    @database_sync_to_async
    def get_session(self):
        from interviews.models import InterviewSession
        try:
            return InterviewSession.objects.get(id=self.session_id, user=self.user)
        except InterviewSession.DoesNotExist: return None
    ...
    def get_last_question(self):
        msg = self.session.messages.filter(role='interviewer').last()
        return msg.content if msg else None

    @database_sync_to_async
    def get_conversation_history(self):
        messages = self.session.messages.all().order_by('timestamp')
        return [{'role': msg.role, 'content': msg.content} for msg in messages if msg.role in ['interviewer', 'candidate']]
    ...
    def save_candidate_message(self, content, score, feedback):
        from interviews.models import Message
        return Message.objects.create(session=self.session, role='candidate', content=content, score=score, feedback=feedback)

    @database_sync_to_async
    def increment_questions(self):
        from interviews.models import InterviewSession
        InterviewSession.objects.filter(id=self.session_id).update(questions_asked=self.session.questions_asked + 1)

    @database_sync_to_async
    def end_session(self):
        from django.utils import timezone
        from interviews.models import InterviewSession
        InterviewSession.objects.filter(id=self.session_id).update(status='completed', ended_at=timezone.now())